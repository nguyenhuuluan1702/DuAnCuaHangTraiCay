...
namespace QuanLyBanTraiCay.Class
{
    internal class Functions
    {
        ...
        public static void Connect()
        {
            ...
            Con.ConnectionString = "Data Source=DESKTOP-1R2Q7BD;Initial Catalog=Quanlybantraicay3;Integrated Security=True";
            Con.Open();                  //Mở kết nối
            //Kiểm tra kết nối
        ...
        {
            string str = "Data Source=DESKTOP-1R2Q7BD;Initial Catalog=Quanlybantraicay3;Integrated Security=True";
            try
            {
                using (SqlConnection con = new SqlConnection( str ))
                {
                    ...
                    // Thiết lập ValueMember và DisplayMember trước khi thiết lập DataSource
                    cbo.ValueMember = ten;    // Trường giá trị
                    cbo.DisplayMember =ma ; // Trường hiển thị

                    // Kiểm tra xem DataTable có dữ liệu hay không
                    if (table.Rows.Count > 0)
                    {
                        // Nếu có dữ liệu, gọi ResetBindings để cập nhật DataSource và hiển thị dữ liệu đúng
                        cbo.ResetBindings();
                    }
                    else
                    {
                        // Không có dữ liệu, bạn có thể thực hiện các hành động khác tùy thuộc vào yêu cầu của bạn
                        Console.WriteLine("Không có dữ liệu để hiển thị trong ComboBox.");
                    }
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine("Error in FillCombo: " + ex.Message);
            }
        }
